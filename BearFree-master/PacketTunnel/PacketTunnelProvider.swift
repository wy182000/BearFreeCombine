...
class PacketTunnelProvider: NEPacketTunnelProvider {
    ...
    override func startTunnel(options: [String : NSObject]?, completionHandler: @escaping (Error?) -> Void) {
        ...
        let mmdbPath = Bundle.main.path(forResource: "GeoLite2-ASN", ofType: "mmdb")
        print("fielPath:" + mmdbPath!)
        GeoIP.database = MMDB(mmdbPath!)!
        ...
        let ipv4Settings = NEIPv4Settings(addresses: ["198.88.1.2"], subnetMasks: ["255.255.255.0"])
        ...
        if enablePacketProcessing {
            let DNSSettings = NEDNSSettings(servers: ["198.88.0.1"])
        ...
        setTunnelNetworkSettings(networkSettings) {
            ...
            if self.enablePacketProcessing {
                ...
                let fakeIPRange = try! IPRange(startIP: IPAddress(fromString: "198.88.1.1")!, endIP: IPAddress(fromString: "198.88.255.255")!)
                let fakeIPPool = IPPool(range: fakeIPRange)
                
                let dnsServer = DNSServer(address: IPAddress(fromString: "198.88.0.1")!, port: NEKit.Port(port: 53), fakeIPPool: fakeIPPool)
